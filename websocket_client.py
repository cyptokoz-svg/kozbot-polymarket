...
class OrderbookSnapshot:
    ...
    def from_message(cls, msg: Dict[str, Any]) -> "OrderbookSnapshot":
        ...
        Polymarket uses 'buys' and 'sells' field names in book events.
        """
        # Polymarket API uses 'buys' and 'sells', not 'bids' and 'asks'
        bids = [
            OrderbookLevel(price=float(b["price"]), size=float(b["size"]))
            for b in msg.get("buys", msg.get("bids", []))  # Try both for compatibility
        ]
        asks = [
            OrderbookLevel(price=float(a["price"]), size=float(a["size"]))
            for a in msg.get("sells", msg.get("asks", []))  # Try both for compatibility
        ]
...
class MarketWebSocket:
    ...
    def __init__(
        ...
        Args:
            ...
            clob_client: Optional ClobClient for REST API orderbook fetching
        """
        self.url = url
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Prepare socket manually to enforce IPv4 and avoid asyncio handshake timeouts
            import socket
            sock = await asyncio.to_thread(self._create_socket)
            ...
            common_kwargs = {
                ...
                "ping_timeout": 30.0,
                "sock": sock,  # Inject our pre-connected socket
            }
            
            try:
                # Try legacy parameters first (extra_headers)
                logger.info("Connecting with legacy parameters (extra_headers) and injected socket...")
                ...
                logger.info(f"Legacy connection failed ({e}), trying modern parameters (additional_headers)...")
                # TypeError occurs during parameter validation before socket is used,
                # so socket should still be valid. No need to recreate.
                
                # Try modern parameters (additional_headers)
                self._ws = await self._ws_connect(
                    self.url,
                    **common_kwargs,
    ...
    def _create_socket(self):
        """Helper to create a connected TCP socket."""
        import socket
        ...
        sock.settimeout(10) # 10s timeout for TCP connect
    ...
    async def subscribe(self, asset_ids: List[str], replace: bool = False, fetch_initial: bool = True) -> bool:
        """
        ...
        Args:
            ...
            fetch_initial: If True, fetch initial orderbook via REST API
        ...
        if replace:
            ...
            logger.info(f"Cleared orderbook cache for {len(asset_ids)} new assets")
        ...
        # Fetch initial orderbook via REST API if available
        if fetch_initial and self.clob_client and replace:
            logger.info(f"Fetching initial orderbooks via REST API for {len(asset_ids)} assets...")
            await self._fetch_initial_orderbooks(asset_ids)
    ...
    async def _fetch_initial_orderbooks(self, asset_ids: List[str]) -> None:
        """Fetch initial orderbook snapshots via REST API."""
        if not self.clob_client:
            return
        
        for asset_id in asset_ids:
            try:
                # Run synchronous REST call in thread pool
                book_data = await asyncio.to_thread(self.clob_client.get_order_book, asset_id)
                
                if book_data:
                    # Convert REST API response to OrderbookSnapshot
                    snapshot = OrderbookSnapshot.from_message(book_data)
                    self._orderbooks[asset_id] = snapshot
                    logger.info(f"✅ Fetched initial orderbook for {asset_id[:20]}...: mid={snapshot.mid_price:.4f}")
                else:
                    logger.warning(f"⚠️ No orderbook data from REST API for {asset_id[:20]}...")
            except Exception as e:
                logger.error(f"Failed to fetch orderbook for {asset_id[:20]}...: {e}")
        ...
        while self._running and self.is_connected:
            try:
                ...
                if msg_count <= 5 or msg_count % 1000 == 0:
                    ...
                    print(f"DEBUG: WS Received raw: {message[:100]}...") # Force print
    ...
    async def run(self, auto_reconnect: bool = True) -> None:
        ...
        while self._running:
            ...
            if self._subscribed_assets:
                ...
                # Wait for initial orderbook data to arrive (fixes race condition)
                # This prevents trading with stale/empty orderbooks after reconnection
                logger.info("Waiting 2s for initial orderbook data...")
