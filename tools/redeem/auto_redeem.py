...
def main():
    ...
    auto_redeem_enabled = bool(config.get("auto_redeem_enabled", False) or os.getenv("AUTO_REDEEM_ENABLED") == "1")
    if auto_redeem_enabled:
        ...
        for pos in redeemable:
            ...
            try:
                ...
                print(f"{status} Redeem {condition_id[:10]}... via {result.get('method')}")
            except Exception as e:
                print(f"❌ Redeem failed for {condition_id[:10]}...: {e}")
    else:
        print("\n注意: Polymarket 的已结算仓位通常会自动转为 USDC")
        print("如果没有自动转换，请在 polymarket.com 手动赎回")
        print("要启用自动赎回，请设置 config.json 的 auto_redeem_enabled=true 或环境变量 AUTO_REDEEM_ENABLED=1")
